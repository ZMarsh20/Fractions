class Fraction:
    ...
    def __displayProper(self):
        ...
        if whole < 0 and abs(self.num + self.den) < self.den:
            whole += 1
        if whole == 0:
    ...
    def add(self, fraction, reduce=False):
        if isinstance(fraction, int) or isinstance(fraction, float):
            self.num += self.den * fraction
        else:
            self.num *= fraction.den
            self.num += (fraction.num * self.den)
    ...
    def __add__(self, other):
        ...
        if fraction.decimal().is_integer():
            return int(fraction.decimal())
        else:
            return fraction
    __radd__ = __add__

    def subtract(self, fraction, reduce=False):
        self.add(-fraction, reduce)
    ...
    def divide(self, fraction, reduce=False):
        if isinstance(fraction, int) or isinstance(fraction, float):
            newFraction = Fraction(1, fraction)
        else:
            newFraction = Fraction(fraction.den, fraction.num)
    ...
    def exponent(self, fraction, reduce=False):
        if isinstance(fraction, int) or isinstance(fraction, float):
            ...
            self.num = pow(self.num, fraction.num)
            self.num = pow(self.num, -fraction.den)
            self.den = pow(self.den, fraction.num)
